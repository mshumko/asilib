...
A ubiquitous way to visualize ASI images and identify different types of aurora is with keograms---an image of pixel intensity as a function of latitude and time. It is assembled by slicing and concatenating pixels that are oriented North-South through zenith (or through a custom path such a path of a stellite) in each image. This is done with the \verb|asilib.plot_keogram()| function (that in turn calls \verb|asilib.keogram()|). Similar to \verb|asilib.plot_map()|, \verb|asilib.plot_keogram()| takes an optional \verb|map_alt| keyword argument. If it is not provided, the keogram's vertical axis is pixel index, but if a valid map altitude is provided, the vertical axis is geographic latitude. To minimize the PC's memory usage, \verb|asilib.keogram()| calls \verb|asilib.load_image_generator()| to load and process one image file at a time. Figure \ref{fig3} is a keogram showing the movement of the auroral arc shown in Fig. \ref{fig2}a.

\begin{figure}
      \includegraphics[width=\textwidth]{figures/fig3.png}
      \caption{A keogram of the auroral arc shown in Fig. \ref{fig2}a.}
      \label{fig3}
\end{figure}
