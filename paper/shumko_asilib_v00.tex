...
Animating just the raw fisheye images is somewhat limiting, thus \verb|asilib| also includes \verb|asilib.plot_movie_generator()| (technically a coroutine) to iterate over customizable ASI images during which you can modify each image. After all of the images are iterated over, \verb|asilib.plot_movie_generator()| combines the modified images into a movie. What sort of modifications can you make? For example you can use it to superpose an overhead satellite path (a conjunction) through each image as described in Section \ref{satellite_conjunction}.
