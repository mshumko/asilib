...
def gps_footprint(gps_dict, alt=110, hemi_flag=0):
    ...
    """
    first_key = list(gps_data.keys())[0]
    if 'interpolated_times' not in gps_dict[first_key]:
        raise ValueError("gps_dict must have 'interpolated_times' before calling gps_footprint()")
    ...
    for sc_key in list(gps_data.keys()):
        n = len(gps_dict[sc_key]['interpolated_times'])
        _all = np.zeros((n, 3), dtype=float)

        time_loc = pd.DataFrame(data={
            'time':gps_dict[sc_key]['interpolated_times'], 
            'x1':(gps_dict[sc_key]['Rad_Re']-1)*R_E,
            'x2':gps_dict[sc_key]['Geographic_Latitude'], 
            'x3':gps_dict[sc_key]['Geographic_Longitude'],
            })
        ...
        gps_dict[sc_key]['footprint_lat'] = lla[:, 0]
        gps_dict[sc_key]['footprint_lon'] = lla[:, 1]
        gps_dict[sc_key]['footprint_alt'] = lla[:, 2]
    return gps_dict
...
def plot_aacgm_grid(ax, time, lat_bounds=(0, 90), lon_bounds=(-150, 0), grid_res=51, alt=110):
...
def init_map(time, center, sat_height_m):
    ...
    plot_aacgm_grid(ax, time=time, alt=alt)
    plt.tight_layout()
    return fig, ax

def interpolate_gps_pos(time_range, gps_data):
    ...
    for sc_key in gps_data:
        ...
        for llr_key in ['Geographic_Latitude', 'Geographic_Longitude', 'Rad_Re']:
            gps_data[sc_key][llr_key] = np.interp(
                interp_times_numeric,
                matplotlib.dates.date2num(gps_data[sc_key]['time']),
                gps_data[sc_key][llr_key],
                left=np.nan,
                right=np.nan,
            )
            if interpolated_jump_indices.shape[0] > 0:
                gps_data[sc_key][llr_key][interpolated_jump_indices] = np.nan
    return gps_data

def remove_gps_units_not_in_view(gps_data, ax):
...
if __name__ == '__main__':
    ...
    flux_color_bounds = (1E4, 1E7)

    marker_cmap = plt.cm.turbo
    marker_cmap.set_bad(color='white', alpha=0.5)

    fig, ax = init_map(time_range[0], center=center, sat_height_m=sat_height_m)
    ...
    gps_data = interpolate_gps_pos(time_range, gps_data)
    gps_data = gps_footprint(
        gps_data, 
        alt=alt, 
        # hemi_flag=1 for northern hemisphere footprint, 0 for same magnetic hemisphere footprint.
        hemi_flag=1
        )
    gps_data = remove_gps_units_not_in_view(gps_data, ax)
    ...
    # Loop through the animation frames, plot the GPS data,
